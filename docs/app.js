...
const HISTORY_SHARDS = (() => {
  // Sharded history: May 2026 -> current month (dynamic, no manual update needed)
  const startYear = 2026, startMonth = 5; // May 2026
  const now = new Date();
  ...
  for (let y = startYear; y <= curYear; y++) {
    ...
    const mEnd = y === curYear ? curMonth : 12;
    for (let m = mStart; m <= mEnd; m++) {
      shards.push(`${y}-${String(m).padStart(2, '0')}`);
    }
  }
  return shards;
})(); // generated dynamically; manual update not needed
const SHARD_CACHE = new Map(); // ym -> rows
...
async function fetchShard(ym) {
  ...
  for (const base of bases) {
    try {
      ...
      const r = await fetch(`${base}/history/${ym}.jsonl`, { cache: 'no-cache', signal: ctrl.signal });
      clearTimeout(tid);
      if (!r.ok) continue;
...
function initCalendar(history) {
  ...
  // Find date range - include sharded months for nav even if not yet loaded
  let allDays = Object.keys(dayMap).sort();
  ...
  async function render() {
    ...
    if (HISTORY_SHARDS.includes(ym) && !SHARD_CACHE.has(ym)) {
      ...
      for (const h of shardRows) {
        const day = h.timestamp.slice(0,10);
        if (!dayMap[day]) dayMap[day] = [];
        if (!dayMap[day].some(x => x.timestamp===h.timestamp)) dayMap[day].push(h);
      }
      allDays = Object.keys(dayMap).sort();
    }
    ...
    // Can navigate? earliest from shards if needed (May 2026 project start)
    const shardEarliest = HISTORY_SHARDS.length ? HISTORY_SHARDS[0] + '-01' : null;
    const earliest = allDays.length > 0 ? allDays[0] : (shardEarliest || `${year}-${String(month + 1).padStart(2, '0')}-01`);
    const earliestFromShards = shardEarliest && new Date(shardEarliest + 'T12:00:00');
    const earliestDate = new Date(earliest + 'T12:00:00');
    // allow nav to shard months even if not yet loaded
    const effectiveEarliest = (earliestFromShards && earliestFromShards < earliestDate) ? earliestFromShards : earliestDate;
    const canPrev = year > effectiveEarliest.getFullYear() || (year === effectiveEarliest.getFullYear() && month > effectiveEarliest.getMonth());
