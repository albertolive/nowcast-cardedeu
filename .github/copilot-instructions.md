...
src/features/   → Feature engineering (208 features: 164 with historical data, 44 real-time only) + wind regime detection
...
**Key pattern — Feedback loop:** Every prediction is logged to JSONL with all 208 `FEATURE_COLUMNS` (including radar/lightning/sentinel values). 60+ min later, the system verifies against actual MeteoCardedeu station data. Verified predictions feed back into the training set on the next retrain cycle. This is how real-time-only features (radar, AEMET, SMC forecast) gradually enter the training data. The model trains on all 208 features from day 1 (44 as NaN); as feedback rows accumulate, XGBoost learns from the newly populated columns automatically.
...
**Key pattern — NWP dominance (reduced):** ~73% of model gain comes from NWP-derived features (down from 84% after colsample_bytree tuning). Top features: `model_predicts_precip` (~54%), `weather_code` (~10%), `precipitation` (~9%), `nwp_rain_amount` (~5%). Forcing feature diversity via `colsample_bytree=0.5` means 50% of trees are built WITHOUT the dominant feature, discovering signal in NWP error detectors (`nwp_rain_persistence_6h`, `nwp_rain_dry_air`, `precip_trend_3h`). To further beat the NWP, independent observation data (radar, lightning) is needed — this requires the feedback loop to accumulate real-time data over months.
...
**Key pattern — Hyperparameter tuning after feature expansion:** Current optimal hyperparams (tuned 2026-03-22 via 2-round grid search): `n_estimators=1200`, `max_depth=7`, `learning_rate=0.012`, `subsample=0.75`, `colsample_bytree=0.5`, `min_child_weight=6`, `gamma=0.15`, `reg_alpha=0.3`, `reg_lambda=2.0`, `early_stopping_rounds=96`. No `scale_pos_weight` — isotonic calibration + threshold search handles class imbalance better than direct upweighting. Key insight: `colsample_bytree=0.5` forces feature diversity — 50% of trees built without `model_predicts_precip`, discovering NWP error patterns in tail features. `max_depth=7` captures complex NWP×surface interactions. Stronger regularization (gamma, reg_alpha, reg_lambda all increased) prevents overfitting with deeper, more diverse trees. Cal F1 improvement: 0.7033→0.7061 (+0.0028), largest single improvement in project history. `eval_metric=logloss` (not aucpr) — optimizes probability quality for isotonic calibration pipeline.
