...
src/features/   → Feature engineering (120 features: 76 with historical data, 44 real-time only) + wind regime detection
...
**Key pattern — Feedback loop:** Every prediction is logged to JSONL with all 120 `FEATURE_COLUMNS` (including radar/lightning/sentinel values). 60+ min later, the system verifies against actual MeteoCardedeu station data. Verified predictions feed back into the training set on the next retrain cycle. This is how real-time-only features (radar, AEMET, SMC forecast) gradually enter the training data. The model trains on all 120 features from day 1 (44 as NaN); as feedback rows accumulate, XGBoost learns from the newly populated columns automatically.

**Key pattern — Physics-based features:** Weather code decomposition (`wc_is_thunderstorm`, `wc_is_rain`, `wc_is_drizzle`) captures different precipitation physics. NWP error detection (`nwp_dry_conflict`, `nwp_wet_conflict`) flags when the NWP model disagrees with surface conditions. `moisture_flux_850` (wind×humidity at 850hPa) measures water transport; `theta_e_deficit` captures convective instability; `cape_change_3h` detects rapid destabilization.

**Key pattern — NWP dominance:** ~82% of model gain comes from just 2 features: `model_predicts_precip` (61.7%) and `weather_code` (20.1%). Both are NWP model outputs. To substantially beat the NWP, independent observation data (radar, lightning) is needed — this requires the feedback loop to accumulate real-time data over months.
...
- **Features:** Register new features in `src/features/engineering.py` and add to `FEATURE_COLUMNS`. All 120 features are in the model — real-time-only features are NaN in historical data but present as columns. XGBoost learns from them as feedback data accumulates.
