...
src/features/   → Feature engineering (199 features: 155 with historical data, 44 real-time only) + wind regime detection
...
**Key pattern — Feedback loop:** Every prediction is logged to JSONL with all 199 `FEATURE_COLUMNS` (including radar/lightning/sentinel values). 60+ min later, the system verifies against actual MeteoCardedeu station data. Verified predictions feed back into the training set on the next retrain cycle. This is how real-time-only features (radar, AEMET, SMC forecast) gradually enter the training data. The model trains on all 199 features from day 1 (44 as NaN); as feedback rows accumulate, XGBoost learns from the newly populated columns automatically.
...
**Key pattern — Hyperparameter tuning after feature expansion:** Current optimal hyperparams (tuned 2026-03-20): `n_estimators=800`, `max_depth=6`, `learning_rate=0.02`, `subsample=0.8`, `colsample_bytree=0.6`, `min_child_weight=5`, `gamma=0.1`, `reg_alpha=0.2`, `reg_lambda=1.5`, `early_stopping_rounds=75`. No `scale_pos_weight` — isotonic calibration + threshold search handles class imbalance better than direct upweighting (+0.0108 F1 OOF). Stronger regularization (colsample 0.7→0.6, reg_alpha 0.1→0.2, reg_lambda 1.0→1.5) helps generalize with 55% NaN pressure level features (+0.0020 F1 OOF). Per-fold F1 uses threshold search instead of fixed threshold.
