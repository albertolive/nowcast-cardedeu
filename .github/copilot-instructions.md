...
src/features/   → Feature engineering (210 features: 166 with historical data, 44 real-time only) + wind regime detection
...
**Key pattern — Feedback loop:** Every prediction is logged to JSONL with all 210 `FEATURE_COLUMNS` (including radar/lightning/sentinel values). 60+ min later, the system verifies against actual MeteoCardedeu station data. Verified predictions feed back into the training set on the next retrain cycle. This is how real-time-only features (radar, AEMET, SMC forecast) gradually enter the training data. The model trains on all 210 features from day 1 (44 as NaN); as feedback rows accumulate, XGBoost learns from the newly populated columns automatically.
