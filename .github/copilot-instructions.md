...
src/features/   → Feature engineering (192 features: 148 with historical data, 44 real-time only) + wind regime detection
...
**Key pattern — Hyperparameter tuning after feature expansion:** When adding many features (e.g., 160→192), the model needs retuning. Current optimal hyperparams: `n_estimators=800`, `max_depth=6`, `learning_rate=0.02`, `subsample=0.8`, `colsample_bytree=0.7`, `min_child_weight=5`, `gamma=0.1`, `reg_alpha=0.1`, `reg_lambda=1.0`, `early_stopping_rounds=75`. Key lesson: lower `learning_rate` (0.05→0.02) prevents overfitting to noise features, lower `colsample_bytree` (0.8→0.7) forces exploration of diverse feature subsets, and `early_stopping_rounds` must scale proportionally with lower lr (30→75). Without retuning, adding features actually degraded metrics; after retuning, all metrics improved vs the original 160-feature baseline.
