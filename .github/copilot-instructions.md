...
src/features/   → Feature engineering (211 features: 165 with historical data, 46 real-time only) + wind regime detection
...
**Key pattern — Feedback loop:** Every prediction is logged to JSONL with all 211 `FEATURE_COLUMNS` (including radar/lightning/sentinel values). 60+ min later, the system verifies against actual MeteoCardedeu station data. Verified predictions feed back into the training set on the next retrain cycle. This is how real-time-only features (radar, AEMET, SMC forecast) gradually enter the training data. The model trains on all 211 features from day 1 (46 as NaN); as feedback rows accumulate, XGBoost learns from the newly populated columns automatically.
...
Test suite: `pytest tests/` (303 tests). Validation also relies on the feedback loop (verify predictions 60+ min later) and weekly accuracy reports.
