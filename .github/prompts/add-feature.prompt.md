...
- **Physics composites**: Multi-variable interaction features like `orographic_forcing` (wind⊥mountain×humidity), `frontal_passage` (combined pressure+wind+temp changes), `convective_composite` (instability×moisture×shear). These are computed from existing historical variables and available in training data from 2021+.
- **Soil and ocean features**: `soil_moisture_*` (ERA5 archive, historical from 2015), `sst_med` (real-time from Marine API + historical backfill from NOAA ERDDAP OISST v2.1, 2015-present, ~98% coverage), `convective_inhibition` (CIN, Historical Forecast API backfill from April 2021, ~44% coverage). Follow the same guard pattern for partial historical coverage.
